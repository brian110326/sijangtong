...
public class MemberReviewRepositoryImpl
  extends QuerydslRepositorySupport
  implements MemberReviewRepository {
  ...
  public List<Member> writableMembers(Long productId) {
    ...
    // 특정 제품을 구매한 member만 reivew를 달 수 있음 asdfasdfasdfsadfsadfsafsadfsadfsadfsadf
    ...
    JPQLQuery<Member> tuple = query
      .select(member)
      .where(
        member.memberEmail.in(
          (
            JPAExpressions
              .select(order.member.memberEmail)
              .from(order)
              .join(orderItem)
